...
# -------------------------------------------------------------------------------------------------------------------
	...
	# Hoạt động chính của button [Details]
	...
	def dtsHover(event):
		...
		# chưa hover
		if dts_pic.get() == 'Resource/Buttons/Button_details1.png':
			# sẽ đổi button sang trạng thái: đã hover
			dts_pic.set('Resource/Buttons/Button_details2.png')
			photo = PhotoImage(file=dts_pic.get())
			details.config(image = photo)
			details.image = photo
	...
	# hoạt động 2: khi chuột ấn vào button [Details]
	def dtsEvent():
		show_csv.show_detail_csv()

	# tạo biến ảnh [Details]
	dts_pic = StringVar()
	dts_pic.set('Resource/Buttons/Button_details1.png')
	...
	# chuột còn ở trên button
	details.bind('<Enter>', dtsHover)
	# chuột rời button
	details.bind('<Leave>', dtsHover)
	...
	def taskButtonEvent(index, arr):
		...
		if my_to_do_list['Done'].iloc[index] == True: # +  +  +  +  +  +  +  +  +  +  +  +  +  +  +  +  +  +  +
			...
			number_of_gem.set(str(int(number_of_gem.get()) - int(my_to_do_list['Checkpoint'].iloc[index])))
